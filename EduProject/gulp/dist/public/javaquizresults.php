...
<body>
    ...
    <div class="java-header">
        ...
        ?>
        <h1>Java Programming quiz results</h1>
    </div>

    <div class="parent">
        <div class="result">
            <h1>You got <?php echo $numberOfCorrectAnswers ?> out of 12 questions correct!</h1>
            <?php
            if ($numberOfCorrectAnswers >= 7) {
                echo "<h2> Well done! </h2>";
                echo '<div class="parent">';
                echo '<form action="functions/javaquizsubmit.php" method="post">';
                echo '<button name="quizSubmit" class="button blue-button"> Submit and redeem 70 experience! </button>';
                echo '</form>';
                echo '</div>';
            } else {
                echo "<h2> You're getting there, but just not yet! </h2>";
                echo "<h3> Get over at least 7 correct to gain experience! </h3>";
            }
            ?>
        </div>
    </div>
