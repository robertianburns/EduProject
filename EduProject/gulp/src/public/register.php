...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty(trim($_POST["username"]))) {
        ...
        if ($query = $connection->prepare("SELECT id FROM accounts_details WHERE username = ?")) {
            ...
            if ($query->execute()) {
                ...
                echo "ERROR CODE SNAKE: That register request couldn't be performed. Please try again!";
            }
            $query->close();
        } else {
            echo "ERROR CODE LEAF: That register request couldn't be prepared. Please try again!";
        }
    }
    ...
    // Make sure the end user confirms their password.
    ...
    if (empty($usernameError) && empty($passwordError) && empty($passwordConfirmationError)) {
        ...
        // Add the end user's details into the accounts_details table.
        $detailsQuery = $connection->prepare("INSERT INTO accounts_details (username, password) VALUES (?, ?)");
        ...
        // Add the end user's progress (all 0 for Boolean false) into the accounts_progress table.
        $progressQuery = $connection->prepare("INSERT INTO accounts_progress (username, webdev_html_done, webdev_css_done, webdev_javascript_done, webdev_quiz_done, java_intro_done, java_fundamentals_done, java_quiz_done) VALUES (?, 0, 0, 0, 0, 0, 0, 0)");
        $progressQuery->bind_param("s", $enteredUsername);

        // If registration is successful, then...
        if ($detailsQuery->execute() && $progressQuery->execute()) {
            // ...redirect to the login page.
            header("Location: login.php");
        }
        // The SQL query couldn't be executed for some reason...
        else {
            echo "ERROR CODE PANDA: That register request couldn't be processed. Please try again!";
        }
        $detailsQuery->close();
        $progressQuery->close();
        $connection->close();
...
<head>
    ...
    <title>Register - EduProject</title>
        ...
        <div class="signup">
            <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                <div class="textContainer">
                    <label>Create an EduProject account</label>
                </div>
                ...
                <a href="login.php">Have an account? Sign in here!</a>
            </form>
        </div>
    </div>
</body>
