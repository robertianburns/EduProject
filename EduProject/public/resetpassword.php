...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty(trim($_POST["updatedPassword"]))) {
        ...
        $updatedPasswordError = "Your password must have at least eight characters!";
    } else {
        $updatedPassword = trim($_POST["updatedPassword"]);
    }

    // Confirm the end user's new password.
    ...
    if (empty($updatedPasswordError) && empty($confirmPasswordError)) {
        if ($query = $connection->prepare("UPDATE accounts_details SET password = ? WHERE id = ?")) {
            ...
            if ($query->execute()) { // As the password has successfully been updated, destroy the session and redirect the end user to the login page.
                session_destroy();
                header("Location: login.php");
                exit();
            } else {
                echo "ERROR CODE FIRE: That password change request couldn't be performed. Please try again!";
            }
            $query->close();
        } else {
            echo "ERROR CODE WATERFALL: That password change request couldn't be prepared. Please try again!";
        }
    }
    $connection->close();
}
...
<head>
    ...
    <title>Reset Password - EduProject</title>
    ...
    <div class="formContainer">
        <div class="login">
            <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                ...
                <a href="profile.php">I don't want to change it!</a>
            </form>
        </div>
    </div>
</body>
